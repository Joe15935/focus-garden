...
impl SwiftLinker {
    ...
    pub fn link(self) {
        ...
        for package in self.packages {
            ...
            // CommandLineTools can ship the modern SwiftPM layout without
            // xcodebuild being available. Detect the actual artifact, not only Xcode.
            let modern_layout =
                xcode27_products_dir(&out_path, configuration, &format!("lib{}.a", package.name))
                    .is_some();
            let search_path = if xcode27 || modern_layout {
...
/// - package-own symbols only, except Mado's two required SwiftRs lifetime
///   exports in its embedded SwiftRs.o member (retain_object/release_object).
///   Other dependency symbols remain local to avoid duplicate globals.
...
fn globalize_cdecl_symbols(archive: &std::path::Path, package_name: &str) {
    ...
    let mut in_swift_rs_member = false;
    ...
    for line in String::from_utf8_lossy(&nm.stdout).lines() {
        if line.ends_with(':') {
            ...
            if let Some(module) = member.strip_suffix(".o") {
                ...
                // Focus Garden links only Mado as a Swift static product.
                // Its SwiftRs dependency defines these explicit @_cdecl lifetime
                // functions, which Rust's SRObject retain/drop calls directly.
                in_swift_rs_member = pkg == "mado" && norm(module) == "swiftrs";
                continue;
            }
        }
        ...
        *occurrences.entry(name.to_string()).or_insert(0) += 1;
        let required_swift_rs_export =
            in_swift_rs_member && matches!(name, "_retain_object" | "_release_object");
        if (!in_own_member && !required_swift_rs_export) || kind != "t" || !name.starts_with('_') {
            continue;
        }
        let bare = &name[1..];
    ...
    let Some(objcopy) = rustup_llvm_objcopy() else {
        panic!(
            "swift-rs: llvm-objcopy is required for internalized @_cdecl exports;              run `rustup component add llvm-tools` and rebuild"
        );
    };
    ...
    let status = cmd
        .status()
        .expect("swift-rs: failed to start llvm-objcopy");
    assert!(
        status.success(),
        "swift-rs: failed to promote required exports in {}",
        archive.display()
    );
}
