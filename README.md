...
- 网站命中后跳到本机中文提示页；应用命中后会像按 ⌘Q 一样被正常退出（后台播放也会停止）；如果它先弹出“是否保存”，就改为隐藏窗口，不强制结束、不丢未保存内容。

详细文档：[中文使用说明](docs/使用说明.md)、[学习导航](docs/学习导航.md)、[数据与恢复](docs/数据与恢复.md)、[验收记录](docs/TEST_RESULTS.md)、[候选研究](docs/RESEARCH.md)。验收记录是功能状态的依据，不能从 README 宣称直接推导实机 PASS。
